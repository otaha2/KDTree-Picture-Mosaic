...
class KDTree
{
  ...
  public:
    ...
    typename KDTree<Dim>::KDTreeNode* buildTree(KDTreeNode*& subroot, vector<Point<Dim>>& points, int left, int right, int currDim);
  ...
  private:
    ...
    //Used in Helper function for Constructor
    int partition(vector<Point<Dim>>& points, int left, int right, int pivotIndex, int currDim);
    int Qselect(vector<Point<Dim>>& points, int left, int right, int k, int currDim);

    //Helper function for Destructor, Copy Constructor, and Find Nearest Neighbor respectively
    void destroyTree(KDTreeNode*& subroot);
    typename KDTree<Dim>::KDTreeNode* copyHelper(KDTreeNode*& subroot);

    void FNNHelper(KDTreeNode* subroot, const Point<Dim>& target, Point<Dim>& CurBest, int curDim, double& curRad) const;
};
